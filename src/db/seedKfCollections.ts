/**
 * Non-destructive seed for the four Knowledge Futures sample collections:
 *   pubpub-archive (v1.0.0, v1.1.0, v2.0.0), open-grants, climate-observations, pub-notes
 *
 * Unlike src/db/seed.ts (which DELETEs the whole database with --force), this
 * script only INSERTs. It is idempotent and safe to run against a live DB:
 *   - finds-or-creates the `knowledge-futures` org (and an attribution user)
 *   - skips any of the four collections that already exist
 *   - reuses content-addressed record/schema rows (onConflictDoNothing)
 *
 * Run against whatever DATABASE_URL points at:
 *   pnpm tsx src/db/seedKfCollections.ts
 *
 * The record/version/schema payloads below are copied verbatim from seed.ts so
 * the produced collections are byte-identical to a fresh seed.
 */
...
/** Create a version + its versionSchemas + records. Returns the new version id. */
async function createVersion(opts: {
  ...
  pushedBy: string
  schemaEntries: { slug: string; schemaId: string; schemaHash: string }[]
  records: { recordId: string; type: string; data: unknown }[]
}): Promise<void> {
  const hash = computeVersionHash(opts.schemaEntries, opts.records, [], opts.metadata)
  const totalBytes = opts.records.reduce(
  ...
  await insertRecords(version!.id, opts.records)
}

// ---------------------------------------------------------------------------
// Idempotent org / user / collection setup
// ---------------------------------------------------------------------------

/** Find an attribution user (existing admin@underlay.org) or create the seed admin user. */
async function ensureAdminUser(): Promise<string> {
  const existing = await db
    .select({ id: schema.user.id })
    .from(schema.user)
    .where(eq(schema.user.email, 'admin@underlay.org'))
    .limit(1)
  if (existing.length > 0) {
    console.log(`[seed-kf] Using existing admin user ${existing[0]!.id}`)
    return existing[0]!.id
  }
  const adminId = 'seed-admin-user'
  await db
    .insert(schema.user)
    .values({ id: adminId, name: 'Admin', email: 'admin@underlay.org', emailVerified: true })
    .onConflictDoNothing()
  console.log(`[seed-kf] Created admin user ${adminId}`)
  return adminId
}

/** Find or create the knowledge-futures org. Returns its id. */
async function ensureKfOrg(adminUserId: string): Promise<string> {
  const existing = await db
    .select({ id: schema.organization.id })
    .from(schema.organization)
    .where(eq(schema.organization.slug, 'knowledge-futures'))
    .limit(1)
  if (existing.length > 0) {
    console.log(`[seed-kf] Using existing knowledge-futures org ${existing[0]!.id}`)
    return existing[0]!.id
  }
  const kfId = uuidv4()
  await db
    .insert(schema.organization)
    .values({ id: kfId, slug: 'knowledge-futures', name: 'Knowledge Futures', isDefault: false })
  await db
    .insert(schema.member)
    .values({ id: uuidv4(), organizationId: kfId, userId: adminUserId, role: 'owner' })
    .onConflictDoNothing()
  console.log(`[seed-kf] Created knowledge-futures org ${kfId}`)
  return kfId
}

/** Create a collection if it doesn't already exist. Returns {id, created}. */
async function ensureCollection(
  ...
  const existing = await db
    ...
    .where(and(eq(schema.collections.organizationId, kfId), eq(schema.collections.slug, slug)))
    .limit(1)
  if (existing.length > 0) {
    console.log(`[seed-kf] SKIP knowledge-futures/${slug} (already exists)`)
    return { id: existing[0]!.id, created: false }
  }
  const id = uuidv4()
  await db.insert(schema.collections).values({ id, organizationId: kfId, slug, name, public: true })
  return { id, created: true }
}

async function main() {
  console.log('[seed-kf] Non-destructive KF collection seed starting...')
  const adminId = await ensureAdminUser()
  const kfId = await ensureKfOrg(adminId)

  // ===================== Collection 1: PubPub Archive =====================
  {
    const { id: pubpubId, created } = await ensureCollection(
      kfId,
      'pubpub-archive',
      'PubPub Archive',
    )
    if (created) {
      ...
      const pubpubRecords = [
        ...
        },
        {
          ...
          data: {
            name: 'Travis Rich',
            orcid: '0000-0001-0503-5905',
            affiliation: 'Knowledge Futures',
          },
        },
        {
          recordId: 'pub-001',
      ...
      await createVersion({
        ...
        pushedBy: adminId,
        schemaEntries: pubpubSchemaEntries,
        records: pubpubRecords,
      })
      console.log('[seed-kf] Created knowledge-futures/pubpub-archive v1.0.0 (17 records)')

      // v1.1.0 (minor: record changes, same schema)
      ...
      await createVersion({
        ...
        pushedBy: adminId,
        schemaEntries: pubpubSchemaEntries,
        records: pubpubV2Records,
      })
      console.log('[seed-kf] Created knowledge-futures/pubpub-archive v1.1.0 (minor)')

      // v2.0.0 (major: schema change — add Review type + abstract field)
      ...
      await createVersion({
        ...
        metadata: pubpubMetadata,
        pushedBy: adminId,
        schemaEntries: pubpubV3SchemaEntries,
        records: pubpubV3Records,
      })
      console.log('[seed-kf] Created knowledge-futures/pubpub-archive v2.0.0 (major)')
    }
  }

  // ===================== Collection 2: Open Grants =====================
  {
    const { id: grantsId, created } = await ensureCollection(
      kfId,
      'open-grants',
      'Open Grants Dataset',
    )
    if (created) {
      ...
      await createVersion({
        ...
        pushedBy: adminId,
        schemaEntries: grantsSchemaEntries,
        records: grantsRecords,
      })
      console.log('[seed-kf] Created knowledge-futures/open-grants (8 records)')
    }
  }

  // ===================== Collection 3: Climate Observations =====================
  {
    const { id: climateId, created } = await ensureCollection(
      kfId,
    ...
    if (created) {
      ...
      await createVersion({
        ...
        pushedBy: adminId,
        schemaEntries: climateSchemaEntries,
        records: climateRecords,
      })
      console.log('[seed-kf] Created knowledge-futures/climate-observations (12 records)')
    }
  }

  // ===================== Collection 4: Pub Notes =====================
  {
    const { id: pubnotesId, created } = await ensureCollection(kfId, 'pub-notes', 'Pub Notes')
    if (created) {
      ...
      await createVersion({
        ...
        pushedBy: adminId,
        schemaEntries: pubnotesSchemaEntries,
        records: pubnotesRecords,
      })
      console.log('[seed-kf] Created knowledge-futures/pub-notes (7 records)')
    }
  }

  console.log('[seed-kf] Done.')
  process.exit(0)
}

main().catch((err) => {
  console.error('[seed-kf] Failed:', err)
  process.exit(1)
})
