            ...
            if (tables.length > 0) {
              for (const row of tables[0].values) {
                ...
                if (data.length > 0 && data[0].values.length > 0) {
                  ...
                  const insertSql = `INSERT INTO "${prefix}__${tableName}" (${
          ...
          if (tables.length > 0) {
            for (const row of tables[0].values) {
              ...
              if (data.length > 0 && data[0].values.length > 0) {
                ...
                const sourceLabel = `${lc.ownerSlug}/${lc.slug}`
                const stmt = newDb.prepare(
                  `INSERT INTO "${prefix}__${tableName}" (${
                    cols.map((c: string,) => `"${c}"`).join(', ',)
                  }) VALUES (${placeholders})`,
                )
                for (const r of data[0].values) stmt.run([...r, sourceLabel,],)
    ...
    <div className='flex h-full overflow-hidden'>
      ...
      <div className='w-[420px] min-w-[360px] border-r border-rule flex flex-col bg-parchment overflow-hidden'>
        ...
        <div className='p-3 border-b border-rule'>
                    ...
                    {collectionSearch.length >= 2 && !searchLoading && (
                      <div className='mt-1.5 max-h-40 overflow-y-auto space-y-0.5'>
                        {searchResults.length === 0
                          ...
                          : (
                            searchResults.map((c,) => {
                              const alreadyLoaded = loadedCollections.some((lc,) =>
                                lc.ownerSlug === c.ownerSlug && lc.slug === c.slug
                              )
                              return (
                                <div
                                  key={`${c.ownerSlug}/${c.slug}`}
                                  className={`flex items-stretch text-xs font-mono ${
                        ...
                        : (
                          availableVersions.map((v,) => {
                            const alreadyLoaded = loadedCollections.some(
                              (lc,) =>
                                lc.ownerSlug === selectedForVersion.ownerSlug && lc.slug === selectedForVersion.slug
                                && lc.version === v.number,
                            )
                            return (
                              <button
          ...
          {/* Loading indicator */}
          {loading && <div className='text-[11px] font-mono text-ink-muted animate-pulse mb-2'>{loadingMessage}</div>}

          {/* SQL / natural language input (CodeMirror) */}
          <div
            ref={editorContainerRef}
            className={`w-full border border-rule text-sm font-mono ${
              loadedCollections.length === 0 || isRunning ? 'opacity-50 pointer-events-none' : ''
            }`}
        ...
        <div className='flex-1 overflow-y-auto'>
          ...
          {history.length === 0 && (
            <div className='p-3 text-xs text-ink-muted'>
              {sqlJsError ? <span className='text-red-700'>{sqlJsError}</span> : !sqlJsReady
                ? (
                  'Loading SQL engine...'
                )
              ...
              <div className='flex-1 overflow-auto min-h-0'>
                ...
                {selectedEntry.rawResponse && (
                  <div className='px-4 py-3 border-b border-rule'>
                    <div className='text-[10px] font-mono text-ink-muted uppercase tracking-wide mb-1'>
                      Raw LLM Output
                    </div>
                      ...
                      <thead>
                        <tr className='border-b border-rule'>
                          {selectedEntry.result.columns.map((col, i,) => (
                            <th
                              key={i}
                              className='text-left px-2 py-1.5 font-medium text-ink-light sticky top-0 bg-parchment'
                            >
                              {col}
                            </th>
                          ))}
                          ...
                          <tr key={i} className='border-b border-rule last:border-0 hover:bg-parchment-dark'>
                            {row.map((cell, j,) => (
                              <td key={j} className='px-2 py-1 max-w-xs truncate'>
                                {cell === null ? <span className='text-ink-muted italic'>null</span> : (
                                  String(cell,)
                                )}
                              </td>
                            ))}
