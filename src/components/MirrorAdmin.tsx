    ...
    <div className='space-y-8'>
        ...
        {testResult && (
          ...
          >
            {testResult.ok
              ? (
                <span>
                  ✓ Valid Underlay server — {testResult.collectionCount}{' '}
                  public collection{testResult.collectionCount !== 1 ? 's' : ''} available
                </span>
              )
            ...
            {expandedRun && (() => {
              ...
              return (
                <div className='mt-2 text-xs font-mono bg-parchment-dark border border-rule rounded p-3 max-h-48 overflow-y-auto space-y-0.5'>
                  {run.logs.map((msg, i,) => <div key={i} className='text-ink-muted'>{msg}</div>)}
                </div>
              )
            })()}
          </div>
      ...
      <section className='border border-rule rounded-lg p-5'>
        ...
        <p className='text-sm text-ink-muted mb-4'>
          Scope which accounts are included or excluded from mirroring. Leave both empty to mirror all public
          collections.
        </p>
