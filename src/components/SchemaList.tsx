 ...
 * field tables, plus the owner-only per-type ARK redirect config. Shared by
 * the latest schemas page and the version-pinned one.
 */
    ...
    <>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-ink-muted text-sm font-semibold">
          ...
          {schemasData?.semver && <span className="ml-1 font-normal">in {schemasData.semver}</span>}
        </h2>
      </div>
      ...
      {schemas.length === 0 ? (
        ...
        <div className="space-y-4">
          {schemas.map((s: any) => {
            ...
            return (
              <div key={s.slug} className="border-rule rounded-surface overflow-hidden border">
                {/* Header */}
                <div className="bg-parchment-dark border-rule flex items-center justify-between border-b px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    {isPrivate && (
                      <span className="border-rule text-ink-muted rounded-control border px-1.5 py-0.5 text-[11px]">
                        private
                      </span>
                    )}
                  </div>
                    ...
                    {labels.length > 0 && (
                      <div className="flex items-center gap-1">
                        {labels.map((label: string) => (
                          <span
                            key={label}
                            className="bg-parchment border-rule text-ink-muted rounded-control border px-1.5 py-0.5 text-[11px]"
                          >
                            {label}
                          </span>
                        ))}
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {fields.map(([name, def]: [string, any]) => {
                      ...
                      return (
                        <tr key={name} className="border-rule hover:bg-parchment-dark/50 border-t">
                  ...
                  <div className="border-rule bg-parchment-dark/30 border-t px-4 py-3">
                    ...
                    >
                      ...
                      <select
                        ...
                        className="bg-parchment border-rule focus:border-ink rounded-control cursor-pointer border px-2 py-1 text-xs focus:outline-none"
                      >
