...
export default function DocsIntegration() {
  return (
    <DocsLayout title="Integration Guide">
      ...
      <h2>Core Concepts</h2>
      <ul>
        <li><strong>Collection</strong> — A named, versioned body of structured data. Identified by <code>:owner/:slug</code>.</li>
        <li><strong>Version</strong> — An immutable snapshot: JSON Schema + records + file references + metadata. Numbered sequentially.</li>
        <li><strong>Record</strong> — A flat JSON object with an <code>id</code>, a <code>type</code>, and a <code>data</code> payload conforming to the schema.</li>
        <li><strong>File</strong> — A binary blob (PDF, image, etc.) stored by SHA-256 hash. Referenced in records via <code>{fileRef}</code>.</li>
      </ul>
      ...
      <ol>
        ...
        <li>Push a version with <code>base_version</code>, schema (if changed), and record changes</li>
        <li>On <code>409 Conflict</code>, re-fetch latest and retry</li>
      </ol>
      <pre className="bg-ink text-parchment p-3 text-xs overflow-x-auto"><code>{diffPush}</code></pre>

      <h2>Record Format</h2>
      <p>Every record has three fields: <code>id</code> (stable string), <code>type</code> (matches schema), and <code>data</code> (the payload).</p>
      <ul>
        <li>Relationships are plain ID strings (e.g. <code>"authorId": "author-1"</code>)</li>
        <li>Files are referenced as <code>{fileRef}</code></li>
      ...
      <ul>
        ...
        <li>Each row becomes a record (primary key → record <code>id</code>)</li>
        <li>Foreign keys become string ID references</li>
        <li>Binary columns (BLOBs) → upload as files, replace with <code>$file</code> references</li>
      ...
      <ul>
        <li>Schema changes → <strong>major</strong> bump</li>
        <li>Record changes → <strong>minor</strong> bump</li>
        <li>Metadata-only changes → <strong>patch</strong> bump</li>
      </ul>
      <p>Version 1 is always <code>v1.0.0</code>.</p>

      <h2>Privacy</h2>
      <p>You can control what's publicly visible at three levels:</p>
      <ul>
        <li><strong>Private types:</strong> Add <code>"private": true</code> to a type in the schema. All records of that type are hidden from public readers.</li>
        <li><strong>Private fields:</strong> Add <code>"private": true</code> to a field in the schema. That field is stripped from public responses.</li>
        <li><strong>Private records:</strong> Add <code>"private": true</code> to individual records when pushing. Those records are hidden from public queries.</li>
      </ul>
      ...
      <table>
        <tbody>
          ...
          <tr><td><code>POST .../versions/upload</code></td><td>Start chunked upload (for large pushes)</td></tr>
          <tr><td><code>GET .../versions/latest</code></td><td>Get latest version</td></tr>
          <tr><td><code>GET .../versions/:n/records</code></td><td>Get records</td></tr>
          <tr><td><code>PUT .../files/:hash</code></td><td>Upload a file</td></tr>
          <tr><td><code>GET /api/collections</code></td><td>Browse public collections</td></tr>
        </tbody>
      </table>
      ...
      <ol>
        <li><strong>Start session:</strong> <code>POST .../versions/upload</code> with metadata (base_version, schemas, message). Returns a <code>sessionId</code>.</li>
      ...
      <ul>
        <li><code>409 Conflict</code> — Another version was pushed since your <code>base_version</code>. Re-fetch and retry.</li>
        <li><code>422 Unprocessable</code> — Records reference files that haven't been uploaded. Upload them first.</li>
        <li><code>400 Bad Request</code> — Schema validation failed or hash mismatch on file upload.</li>
      </ul>

      <h2>Source Code</h2>
      <p>Underlay is open source: <a href="https://github.com/knowledgefutures/underlay">github.com/knowledgefutures/underlay</a></p>
      <p>Built by <a href="https://www.knowledgefutures.org">Knowledge Futures</a>, a 501(c)(3) nonprofit. Contact: <a href="mailto:team@knowledgefutures.org">team@knowledgefutures.org</a></p>
    </DocsLayout>
  )
}
