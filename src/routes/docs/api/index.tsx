...
export default function DocsApi() {
  return (
    <DocsLayout title="API Overview">
      ...
      <p>All <code>GET</code> requests are <strong>public</strong> — no authentication required to read public data. All write requests (<code>POST</code>, <code>PATCH</code>, <code>PUT</code>, <code>DELETE</code>) require authentication.</p>
      ...
      <ul>
        <li><code>read</code> — list and download data</li>
      ...
      <p>If a <code>Bearer</code> token is provided but does not match any key, the request is <strong>immediately rejected</strong> with <code>401</code> — it will not fall through to anonymous access.</p>
      ...
      <p>When you exceed the limit, you'll receive a <code>429 Too Many Requests</code> response with a <code>Retry-After</code> header indicating how long to wait.</p>
      <p>For any automated or scripted access, <strong>always use an API key</strong> to get the higher rate limit.</p>

      <hr className="border-rule my-6" />

      <h2>Error Responses</h2>
      <p>Errors return a JSON body with <code>error</code> and <code>statusCode</code>:</p>
  ...
  "statusCode": 401
      ...
      <ul>
        ...
        <li><code>401</code> — Authentication required or invalid credentials</li>
        <li><code>403</code> — Insufficient permissions (wrong scope)</li>
        <li><code>404</code> — Resource not found</li>
        <li><code>409</code> — Version conflict (re-fetch and retry)</li>
        <li><code>422</code> — Validation error (e.g. missing files)</li>
        <li><code>429</code> — Rate limited (wait and retry)</li>
      </ul>
