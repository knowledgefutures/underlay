  ...
  return (
    <BaseLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {version.message && (
          <p className="text-sm text-ink-muted mb-4">{version.message}</p>
        )}
        ...
        <div className="flex items-center gap-0 border-b border-rule mb-6">
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === 'records' ? 'border-ink text-ink' : 'border-transparent text-ink-muted hover:text-ink hover:border-rule'}`}
          >
            Records{' '}
          ...
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === 'schema' ? 'border-ink text-ink' : 'border-transparent text-ink-muted hover:text-ink hover:border-rule'}`}
          >
            Schema
          </button>
          <button
            onClick={() => setTab('files')}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === 'files' ? 'border-ink text-ink' : 'border-transparent text-ink-muted hover:text-ink hover:border-rule'}`}
          >
            Files{' '}
            <span className="text-ink-muted font-normal">
          ...
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === 'metadata' ? 'border-ink text-ink' : 'border-transparent text-ink-muted hover:text-ink hover:border-rule'}`}
          >
            Metadata
          </button>
        </div>
                          ...
                          >
                            ...
                            {currentTypeFields.map((f: string) => {
                              ...
                              if (val && typeof val === 'object' && '$file' in val) {
                                ...
                                const fileUrl = `https://assets.underlay.org/files/${hash.slice(0, 2)}/${hash.slice(2, 4)}/${hash}`
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-2.5 text-right">
                              <a
                                href={`https://assets.underlay.org/files/${f.hash.slice(0, 2)}/${f.hash.slice(2, 4)}/${f.hash}`}
                                target="_blank"
                                className="text-link hover:underline"
                              >
                                <svg
