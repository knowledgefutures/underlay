...
export default function OwnerSettingsKeys() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/accounts/${owner}/keys`, { credentials: 'include', },).then((r,) => r.ok ? r.json() : []),
    ...
    <BaseLayout>
      <div className='max-w-4xl mx-auto px-4 py-10'>
        ...
        <div className='text-xs text-ink-muted bg-parchment border border-rule px-3 py-2 mb-4'>
          <strong className='text-ink'>Rate limits:</strong>{' '}
          Authenticated requests get 5,000 req/min. Without a key, the API allows 60 req/min per IP.
                  ...
                  <div>
                    ...
                    <div className='flex items-center gap-2 mt-0.5 text-xs text-ink-muted flex-wrap'>
                      ...
                      {k.expiresAt && !isExpired(k.expiresAt,) && (
                        <span
                          className={isExpiringSoon(k.expiresAt,) ? 'text-yellow-700 font-medium' : ''}
                        >
                          · Expires {new Date(k.expiresAt,).toLocaleDateString()}
                        </span>
                      )}
                      {isExpired(k.expiresAt,) && <span className='text-red-700 font-medium'>· Expired</span>}
                    </div>
                  </div>
                  {isAdmin && (
                    <button
