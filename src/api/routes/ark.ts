...
export async function arkRoutes(app: FastifyInstance) {
  // --- Resolution ---

  app.get("/ark/resolve", async (request, reply) => {
    ...
    if (arkLabelIdx === -1) return reply.status(400).send({ error: "Invalid ARK path" });

    const afterLabel = path.slice(arkLabelIdx + 4); // strip "ark:"
    const slashIdx = afterLabel.indexOf("/");
    if (slashIdx === -1) return reply.status(404).send({ type: "not_found" });
    ...
    if (!pathAfterNaan) return reply.status(404).send({ type: "not_found" });

    const components = parseArkPath(pathAfterNaan);
    if (!components) return reply.status(404).send({ type: "not_found" });
    ...
    const [shoulderRow] = await db
      ...
      .limit(1);
    if (!shoulderRow) return reply.status(404).send({ type: "not_found" });
    ...
    if (!collRow || !collRow.enabled) return reply.status(404).send({ type: "not_found" });

    // Verify the shoulder belongs to the collection's owner
    if (shoulderRow.accountId !== collRow.collectionAccountId) {
      return reply.status(404).send({ type: "not_found" });
    }
    ...
    if (version !== undefined) {
      const [row] = await db
        ...
        .limit(1);
      if (!row) return reply.status(404).send({ type: "not_found" });
      versionRow = row;
    } else {
      const [row] = await db
        .select({
    ...
    if (recordType && recordId) {
      ...
      if (!artRow) return reply.status(404).send({ type: "not_found" });

      if (!versionRow) return reply.status(404).send({ type: "not_found" });
      ...
      if (!recordRow) return reply.status(404).send({ type: "not_found" });

      const data = recordRow.data as Record<string, unknown>;
      const redirectUrl = data[artRow.redirectUrlField];
      if (typeof redirectUrl !== "string") {
        return reply.status(404).send({ type: "not_found", error: "No URL found for this record" });
      }
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const hasAccess = await checkCollectionAccess(coll.accountId, request.accountId!);
      ...
      if (!arkRow) {
        return { enabled: false, customUrl: null, arkUrl: null, shoulder: null, arkId: null };
      }

      const arkUrl = buildArkUrl(naan, arkRow.shoulder, arkRow.arkId);
      return { enabled: arkRow.enabled, customUrl: arkRow.customUrl, arkUrl, shoulder: arkRow.shoulder, arkId: arkRow.arkId };
    },
  );
  ...
  app.patch(
    ...
    async (request, reply) => {
      const { owner, slug } = request.params as { owner: string; slug: string };
      const { enabled, customUrl } = request.body as { enabled?: boolean; customUrl?: string | null };
  ...
  app.get(
    "/collections/:owner/:slug/ark/record-types",
    { preHandler: [requireAuth("read")] },
    async (request, reply) => {
      const { owner, slug } = request.params as { owner: string; slug: string };
  ...
  app.patch(
    ...
    async (request, reply) => {
      const { owner, slug } = request.params as { owner: string; slug: string };
      const { recordType, redirectUrlField } = request.body as {
        recordType: string;
        redirectUrlField: string | null;
      };

      if (!recordType) return reply.status(400).send({ error: "recordType required" });
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const { naan } = request.body as { naan: string | null };

      if (naan !== null && !/^\d{1,16}$/.test(naan)) {
        return reply.status(400).send({ error: "NAAN must be numeric (up to 16 digits)" });
      }
      ...
      } else if (account.id !== request.accountId) {
        return reply.status(403).send({ error: "Forbidden" });
      }
