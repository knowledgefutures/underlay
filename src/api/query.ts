...
export async function generateSql(c: Context<AuthEnv>,) {
  ...
  if (collectionRefs.length === 1) {
    ...
    if (!result) return c.json({ error: `Collection ${ref.owner}/${ref.slug} v${ref.version} not found`, }, 404,)
  ...
  const systemPrompt =
    `You are a SQL assistant for SQLite databases. Given a schema and a user's question, produce a single SELECT query that answers it.
...
- Do NOT include columns that don't exist in the schema.`
  ...
  try {
    ...
    if (!raw) {
      return c.json({ error: 'LLM returned empty response', rawResponse: JSON.stringify(data,), }, 500,)
    }
