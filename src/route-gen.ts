...
export function buildDataRoutes(globResult: Record<string, () => Promise<any>>): RouteObject[] {
  const entries = buildRoutes(globResult)

  return entries.map((entry) => ({
    path: entry.path,
    lazy: async () => {
      const mod = await globResult[entry.filePath]!()
      return {
        Component: mod.default as ComponentType,
        ...(mod.handle && { handle: mod.handle }),
      }
    },
  }))
}
